...
#     2) Invokes the TellDateTimeTimezoneAgent to fetch the current time
...
class GreetingAgent:
    ...
    🧠 Orchestrator “meta-agent” that:
      ...
      - On a “greet me” request:
          ...
          2) Calls call_agent("TellDateTimeTimezoneAgent", "What is the current time?")
    ...
    def _build_orchestrator(self) -> LlmAgent:
        ...
        system_instr = (
            ...
            "call_agent('TellDateTimeTimezoneAgent','What is the current date, time and timezone?'), "
            "then craft a 2–3 line poetic greeting referencing that date, time and timezone."
        )
...
if __name__ == "__main__":
    async def main():
        agent = GreetingAgent()
        result = await agent.invoke("Greet me", session_id="test-session-123")
        print("\n=== GreetingAgent reply ===")
        print(result)
